...
- [docs/016_book-interactions.md](docs/016_book-interactions.md) — amended implementation-grade proposal: comments + inline comments with workspace-only ordinary moderation authority, rate limits and block-orphaning; bookmarks + reading progress as subject-private state retained with target redaction after revoked access (depends on docs/015)
- [docs/017_epub-import.md](docs/017_epub-import.md) — amended implementation-grade proposal: bounded asynchronous EPUB processing via `workers/epub-processor/`, normalized `book_import_items`, least-privilege import execution, import-specific content/media writes, derived-count finalization and `/replace` integration; `book_imports` remains operational source of truth (depends on docs/015)
- [docs/018_review-batch-015-017.md](docs/018_review-batch-015-017.md) — review complete: preserves original findings, records product-decision corrections, and explains the targeted amendments incorporated into docs/015–017
