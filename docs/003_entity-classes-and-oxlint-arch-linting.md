...
5. Architecture linting runs inside oxlint as a JS plugin with 16 rules, matching the behavior of the old script and adding guards against raw entity serialization and route-handler boundary drift.
...
| Rules | Hardcoded in one script | 16 independent rules, configurable severity |
...
4. Write the oxlint plugin with all 16 rules.
...
| `route-handler-boundary` | Route handlers must stay thin: no direct `c.env`, global `fetch`, `crypto`, `JSON.parse`/`JSON.stringify`, direct storage calls, or manual `Request`/`Response` construction | `src/http/routes/*.routes.ts` |
...
- Enabled all 16 `architecture/*` rules at `"error"` severity.
...
- **Route and persistence shortcuts**: Route validation is checked against any handler parameter name, not just `c`; protected route security must be the exact `bearerSecurity` helper; route handlers cannot do low-level env/fetch/crypto/JSON/storage/Response work; repository/workflow writes must use `CrudAdapter`, with `db.batch(...)` limited to workflow ports.
